...
const handleExport = async (project: ProjectSummary) => {
  ...
  exporting.value = project.id;
  exportProgress.value = null;
  error.value = null;

  try {
    const blob = await exportProject(project.id, (p) => {
    ...
    a.href = url;
    a.download = `${sanitizeFilename(project.name)}.awp`;
...
const handleKeydown = (e: KeyboardEvent) => {
  if (e.key === "Escape" && confirmDeleteId.value) {
    cancelDelete();
  }
};
...
// Expose refresh so parent components can trigger it.
...
<template>
  ...
  >
    ...
    <!-- Import progress -->
    <div v-if="importProgress" class="alert alert-info text-sm py-2">
      <i class="iconify mdi--loading animate-spin size-4" />
      <span>{{ importProgress }}</span>
    </div>

    <!-- Export progress -->
    <div v-if="exportProgress" class="alert alert-info text-sm py-2">
      <i class="iconify mdi--loading animate-spin size-4" />
    ...
    <div
      v-if="dragOver"
      class="flex items-center justify-center py-10 border-2 border-dashed border-primary rounded-lg bg-primary/5"
    >
