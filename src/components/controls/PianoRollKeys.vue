    ...
    <div
      ...
      :class="{
        ...
        'cursor-pointer text-base-content/40 bg-base-300/30 hover:bg-primary/20 hover:text-primary':
          !props.disabled && pitch.key.includes('#') && pressedId !== pitch.key,
        // Any key pressed
        'bg-primary/30 text-primary':
          !props.disabled && pressedId === pitch.key,
        // Disabled natural key (preserve sharp tint distinction)
        'text-base-content/70': props.disabled && !pitch.key.includes('#'),
        'text-base-content/40 bg-base-300/30':
          props.disabled && pitch.key.includes('#'),
      }"
